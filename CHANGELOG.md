...
- `go test -count=1 ./internal/data/documenthubstore/` PASS (84.1% statements against the 70% floor), `go vet` and `gofmt` clean; docs renderer Go tests PASS and both Playwright specs 2/2 in Chromium (desktop + 390px, keyboard paths, no console errors). Fifteen hub todos remain blocked on open CHAT-001/002/010/012/043/052 and INTAPI-007 (dependency table in the devlog entry). Work left uncommitted; the full commit hook and CI remain for a committing session.

## 2026-09-22 (Chat agent API surface)

- Documented the per-conversation machine API for public/private channels, groups, and direct chats: metadata, paged post reads, idempotent post writes, and bounded post-event pulls. Added local port 8888 curl examples and clarified that a manager-approved per-conversation app installation, matching machine identity, and explicit `chat.posts.read` / `chat.posts.write` scopes are required. Production token issuance is not provided; the examples use a placeholder from an authorized issuer. The real-PostgreSQL four-kind authorization and read/write matrix passed three runs, including a 5,000-row history-filter regression; transient database failure returned 503, and revoked/private resources returned indistinguishable 404s. On r9, port 8888 served the 48,591,082-byte WASM (`FC9713D57208D29A2CCE96059CA0D0E0374703B7CC7820B807F6253500063350`; HCM PID 53720, gateway PID 55708); app, WASM, and health returned 200. Browser QA opened the API menu for all four conversation kinds and copied the new curl snippet with a clean console. Production credential issuance remains unavailable; no chat todo was ticked and no files were staged or committed.

## 2026-09-22 (Chat global search)

- Added chat search for authorized channel names, visible people, and matching messages, with independent paging by result category and navigation that focuses the selected message. Fixed a WASM `Illegal invocation` on message selection by binding `requestAnimationFrame` to the window; the focused JS/WASM regression passed.
- r6 review exposed a People-result cache overwrite during fresh searches, so “Anika” initially returned seven messages without Anika Desai. r7 fixes the interleaving in `adoptLoadedChatProjection` with a regression test. Final independent r7 visual QA scored 9/10 with no P1/P2 findings: a fresh session showed People → Anika plus seven messages, `bene` returned #benefits, message navigation focused the exact hit, and post-jump layout stayed responsive and readable at 390px and 320px. The console was empty; viewport state was reset and the QA tab closed. CHAT-021 remains unchecked because its dependencies and full acceptance gate matrix remain open.

## 2026-09-22 (Chat capability and rendering follow-up)

- Added safe Markdown rendering, cursor-preserving emoji pickers in both composers, and skeleton loaders with reserved thread space and reduced-motion support. Draft writes now merge across concurrent sidebar revisions, and users can open a private conversation with themselves.
- Uploaded PNG, JPEG, and BMP media now use protected thumbnail and display renditions; the timeline requests thumbnails near the viewport and the fullscreen viewer loads the display rendition. Animated GIFs remain unchanged. Added a per-channel agent API cURL example with an idempotency key, scoped machine-authenticated post read/write routes, and a tenant admin editor for age/date or size retention settings.
- Focused native chat UI/product UI tests, focused JS/WASM chat UI/journey tests, native and WASM builds, narrow `go vet`, real-PostgreSQL machine API HTTP integration tests, and targeted chat media package tests passed. On final candidate r4, live port 8888 (HCM PID 88108) served WASM SHA-256 `A161203B351EBA2C9FA6ADDA07EAC818692021B110D2B8A4AF9A32D313BED1F9`. Browser QA confirmed an image thumbnail and fullscreen viewer, Rafael's profile and Message action, and the self-DM label after reload. A native and executed WASM regression verified replayed reaction-count idempotence; self-DM member copy is localized and singular. The no-key GIPHY state was disabled with an explanatory message; the channel cURL action copied the current-origin URL, token placeholder, and UUID idempotency header.
- Retention settings persist but are not enforced; production machine credential issuance and event replay remain pending; GIPHY search was not exercised because no API key is configured; animated GIFs are not optimized; self-DM lookup still has a scalability gap. Final independent Chrome adversarial QA on r4 scored 9.0/10 with no P1/P2 findings: as Rafael, 33 reaction counts stayed at 1 while opening/closing the fullscreen viewer, jumping to newest, and switching from #announcements to #benefits and back; the self-DM header showed “1 member” and the console had no warnings or errors. Root independently observed 12 visible reaction counts remain stable through viewer and jump interactions. CHAT-001..055 remain unchecked because the individual todo dependencies and acceptance gates are not satisfied by this focused feature verification. The full shared-index gate, coverage, and commit hook remain unverified; no files were staged or committed.

## 2026-09-22 (Chat adversarial interaction refinement)

- Refined message and thread menu keyboard behavior so arrows, Home/End, Escape, Tab and reverse Tab use the actual opener and restore focus predictably. Late projections preserve an open five-reply thread and loading state. Mobile drawer search focus, trapping, inert background and cleanup were verified across close paths. Menu geometry flips, caps and repositions from measured bounds; representative 390 px light and 320 px dark Arabic views stayed within the viewport.
- Two independent source/UI reviews renewed a 9.25/10 score with no release blockers after the 7.5/10 baseline. Root drove final 320 px drawer/search focus, reverse-Tab and opener checks, timeline/thread menu keyboard paths, five-reply stability and clean inert cleanup. R5 fixes the async thread loading race with a framework `ui.PostAsync` lifecycle-fenced tick guarded by mount epoch, generation, room and thread root; the actual DOM regression asserted one synthetic reply text without a second action and stayed clean after unmount. Focused native chat UI, executed WASM menu/drawer, native journey/thread/composer tests, and vet passed. Live port 8868 health returned 200 and served the exact promoted 47,522,155-byte WASM, SHA-256 `F5E869FD11F318AD0FB75B0399F2202D9892D2E5E8241BA78AA487F69774B8CC`. A fresh live Q4 browser smoke showed all five replies without another action, no loading state, and no console warnings/errors; the 0.365-second wait is observational UI timing, not a benchmark.
- One reviewer could not authenticate the final runtime; the other reviewed source and screenshots without independent browser, screen-reader or contrast instrumentation. Remaining minor notes are Tab returning to the opener before a second Tab and dense 320 px header spacing. Shared-index full hook/coverage remains unrun; the unrelated WEB-148 product UI failure persists.

## 2026-09-22 (Copy channel and chat references)

- Conversation row menus now copy a `#Name_With_Underscores` label and same-origin chat URL. Browsers that support rich clipboard data receive an HTML anchor plus a plain-text fallback; other browsers receive `#label URL`. Pasted references render as links in chat drafts, timelines, and threads. Known joined rooms open in place, while unknown links use an authorized server conversation read and show only a generic `#channel` label until access is confirmed. No link grants membership. English, German, and Arabic controls and transient clipboard feedback are included. Native chat UI, focused executed JS/WASM clipboard/parser/draft/fragment tests, and narrow vet passed. Preview QA verified `#general`, `#Q4_hiring_huddle`, and `#Evelyn_Morgan` labels, clickable draft preview, and a fresh-tab channel link. At 320px and 390px, menus fit with no overflow or console errors. Live port 8868 served the promoted 47,443,945-byte WASM with SHA-256 `F72EC3971F28F69997887F0F080401F5A9C25D0782BC03AA2EADF52A5110E35B`; live clipboard read confirmed both plain `#general` plus the 8868 URL and the HTML anchor, and Ctrl+V pasted the plain form. Draft and clipboard were restored after QA; no post was sent. The full shared-index commit hook remains pending.

## 2026-09-22 (Copy chat message contents)

- A fresh tab on the promoted live build showed the Copy message contents menu with no console diagnostics; the temporary QA tab was closed.
- Added Copy message contents to each timeline message menu and to thread parent and reply menus. It copies only the current message's exact plain text, including line breaks; textless messages show a disabled action with a reason. Clipboard success and failure use transient notices in English, German, and Arabic. Focused native chat UI, product UI Chat, executed JS/WASM clipboard, and narrow vet checks passed for timeline and thread lookup, unknown or closed-thread messages, exact multiline text, success, rejection, and unavailable clipboard. Preview QA confirmed exact multiline timeline and thread clipboard text, a visible success notice, first-click mobile menu access at 320px, no overflow at 320px and 390px, and a clean console. Live port 8868 returned HTTP 200 and served the promoted 47,359,447-byte WASM with SHA-256 `8005393279BD40AEFC34E321093A1C3617D183CE358B92924A2AFB50A64DC4CB`. The full shared-index commit hook remains pending.

## 2026-09-22 (Chat message link previews)

- Copy link now asks the server for a message share locator and copies a same-origin chat URL. Pasting it shows a viewer-authorized message preview in the composer, posted timeline, and thread, including source channel, author, text and attachment count. Opening a preview goes to the source channel; an older post may require scrolling or loading earlier history. Links resolve for each reader through `ResolveShareLink`; source text stays in client memory and is never copied into the destination post. Unavailable, deleted and denied messages show a neutral card. English, German and Arabic labels are included.
- Same-origin parsing rejects external lookalikes and accepts old `#msg` links only when the message is already loaded in the selected room. Resolution is bounded to 24 locators with a 30-second freshness window and stale-session guards. Focused native rendering, parser and cache tests and an executed JS/WASM resolver test passed. Dark-mode browser QA verified Copy link → draft preview, opening the source channel from a new direct-link tab, an unavailable link, preserved drafts, and layout without overflow at desktop, 390px and 320px. At 320px, Jump to newest stayed above the expanded composer. The final live port 8868 served WASM SHA-256 `F39FB7D44AFFFA09684F84964857C62AFA6C765037624D085F23740318BD1BF7` with no console errors. No live post was sent during browser QA. Light mode and the full commit hook remain unverified.

## 2026-09-22 (Share message to channel)

- Added Share to channel to a message's More actions menu. A searchable dialog lists fresh, joined public and private channels, previews the source author and text, and requires an explicit destination and Share action. The server's `ForwardPost` checks source disclosure and destination posting rights, derives original-author attribution, and keeps retries idempotent. English, German and Arabic copy, keyboard focus, Escape/Cancel, pending, error and success states are included.
- Forwarding currently copies message text only; the dialog says attachments are not included, and a media-only message cannot be shared. The attachment warning and error alert remain visible outside the scrollable source preview; the final attachment case passed at 1280×720 and 320×740 with reachable actions and no overflow. Fixed a fast-result render race, a filtered-out destination that remained selected, and filtering during a pending send. Focused native and executed JS/WASM tests passed for these states, RPC success and retry, duplicate clicks, stale auth responses and local draft preservation. Dark-mode preview QA passed at 1280×720, 390×844 and 320×740 with no overflow or console diagnostics. The promoted live build reopened Share, loaded 13 eligible channels, and closed with Cancel without console diagnostics; its WASM SHA-256 was `1021192BFDC9F3E04BAAE608A74D938F8BFE2192CB1E94F50570C48E6390FB2B`. No live post was sent during browser QA. Light mode and the full commit hook were not verified.

## 2026-09-22 (Chat timeline memory and media bounds)

- Added variable-height timeline virtualization above 160 messages, capping the active window at 96 rows plus up to four protected controls; lists at or below 160 messages render normally. A 2,000-message fixture rendered 17–29 DOM rows at 1280 px. Append and prepend operations preserved the exact visible message ID and scroll offset; editor caret focus, offscreen parking, and clean unmount/remount behavior were covered.
- Two-sided paging retains seen IDs with a bounded 128-row overlap, recovers older/newer pages including reply-only scans, prunes reaction cache state, and preserves drafts and other intentional user state. Active server pins continue to scale with directory and organization state.
- Media previews enforce a 16 MiB encoded and 32 MiB estimated decoded PNG/JPEG bound with four fetches, a 20-second abort and lifecycle cleanup. Other or oversized media use bounded downloads up to the 64 MiB server limit with one active and two completed retained URLs for at most 30 seconds; room, auth and unmount clear them. Fullscreen preview remains viewport-only.
- GIF previews use the same 32 MiB decoded frame budget; production `image/gif` rendering was verified with `celebration.gif` at natural width 120, while the separate download path saved a 2,271-byte file. Focused native chat UI and journey tests passed, including the 10,200-event and 500-timeline suites, bounded-seen recovery, media lifecycle, and combined WASM controls/media/person coverage. The 2,000-message stress run held its bottom gap at 0 to −1 px and its 71–72 px small up-scroll through rerender and paging; the final app preview/live smoke held 0.33 px and 72.33 px respectively. Prepend/append/re-jump preserved anchors and bottom state. Home → Chat unmount/remount was clean with an empty console. Live port 8868 health returned 200 and served the exact promoted 47,331,035-byte WASM candidate, SHA-256 `3F9E85417D7BF12318ECB3768E6BCF7925D91CA3D44657220D9F1A6CDA894ADE`. The shared-index full hook and coverage remain unrun under the separate coordinator; the full product UI suite has an unrelated WEB-148 failure while focused chat verification passed.

## 2026-09-22 (Chat group creation controls)

- Replaced the persistent, cramped group-name textbox with a full-width “+ New group” disclosure in the conversation rail. The expanded form keeps the localized name label, input, quiet Cancel and primary Create actions; Enter submits, while Cancel and Escape clear the draft, close the disclosure and restore summary focus.
- Opening the form keeps its input and actions visible by adjusting only the nearest rail scrollport based on measured bounds. Draft text survives unrelated room changes, and the document/timeline scroll positions remain unchanged. Added the missing plus icon and preserved English, German and Arabic copy.
- Native chat UI tests and the focused executed JS/WASM disclosure regression passed. Dark-mode browser QA passed at 1280×720, 390×844 and 320×740 with no horizontal overflow or console diagnostics. Light mode was not inspected.

## 2026-09-22 (Chat person details and direct messages)

- Clicking a person's chat name or photo opens an authorized business profile with their full directory name, job title, department, visible manager, location, company and business unit. Phone and email stay empty because the current governed worker directory does not disclose business contact values. The profile's Message action reuses an admitted direct message or creates one with stable participant identity, then opens its composer. New and reused rooms appear in Direct messages with the peer's directory name.
- Focused native and executed JS/WASM tests passed for field mapping, withheld managers, delayed projections, direct-message reuse and creation, duplicate clicks, stale reads, section placement and cold-load room naming; narrow vet passed. Browser QA confirmed profile fields and photos, self-message disabled, an Anika DM created without sending a post, an existing Evelyn DM reused, no horizontal overflow at 390px or 320px, and the attachment viewer unaffected. The combined preview and live port 8868 passed the final retest: Anika Desai and Daniel Nguyen appeared in the DM sidebar after a cold load; selecting Anika kept her name in the header and composer after stream events and after switching from Evelyn; full profile and manager names included Evelyn Morgan/Amina Rahman and Anika Desai/Sofia Petrov; the profile heading received focus; and Message on an already-open DM closed the profile and focused the composer. The New group form opened with its input focused and Cancel closed it. Consoles were clean. The live build hash was `D7CC7CE56B2E387CEFBCAAF5ED5AD1E006D1ED23568A132C11AFC27AA9C4D39C`; the full commit hook remains unverified.

## 2026-09-22 (Chat channel selection stability)

- Repeatedly selecting a channel while its posts are still loading now keeps the pending read, generation, and local chat render intact. A stale read from an earlier A → B → A switch cannot clear the newer A read or drain its queued send. Another channel still opens, and a failed read still allows an explicit retry. Focused native tests and an executed JS/WASM click-path regression passed; the native Go run printed `ok` before the documented Windows temporary-executable cleanup error. Browser QA of ordinary switches, a Sales double click, and rapid Sales → Design → Sales double click settled on Sales with 34 posts, kept the app shell visible, and showed no console diagnostics.

## 2026-09-22 (Chat image viewer)

- Loaded message images open in a viewport-sized viewer using the attachment's existing authorized URL. The viewer fits the full image, supports keyboard and backdrop dismissal, restores focus, and closes when the chat room, signed-in person, or page changes. English, German, and Arabic control labels were added.
- Dark-mode browser review passed at 1280×720, 390×844 and 320×740: the image scales within the viewport, the close control stays visible, and the page has no overflow or console diagnostics. Click and Enter open the viewer; Tab and Shift+Tab stay inside it; Escape, close and backdrop dismiss it with focus restored. Clicking the image leaves it open, and navigating back to Home removes it. Light mode was not inspected.

## 2026-09-22 (Personal chat groups)

- Added named, collapsible groups to the left conversation rail. A conversation's menu can move it between the default sections and personal groups; removing a group returns its conversations to Channels or Direct messages without changing membership. Group names and collapse state persist in the recipient-owned sidebar layout.
- Guarded sidebar reads and serialized writes against stale local edits, and kept group state through late conversation-list refreshes. Added English, German and Arabic controls and focused group rendering, event, and reconciliation regressions. Dark-mode browser review passed at desktop, 390×844 and 320×740; the full commit gate remains pending.

## 2026-09-22 (Chat conversation menus)

- Added a separate vertical three-dot menu to each channel, direct message and group row. View details selects the menu's conversation; All messages, Mentions only and Muted save that conversation's notification preference without changing the current selection. The menu supports keyboard navigation, Escape, outside-click dismissal, focus restoration and English, German and Arabic labels.
- Focused native chat UI tests, vet and targeted JS/WASM notification-save and DOM reconciliation regressions passed. In the dark-mode browser, the menu and details flow passed at desktop, 390×844 and 320×740 with no document overflow or console warnings/errors. Light mode was not inspected.

## 2026-09-22 (Chat profile photos)

- Chat now uses authorized worker profile photo URLs for message and thread authors, members, and direct-message peers resolved from memberships. Missing or failed images leave initials in fixed-size avatar slots; channels and groups retain their existing markers. Late worker directory results survive chat projection refreshes. The browser revealed a deferred DOM panic from image handlers wrapped before runtime initialization; handlers now initialize once when first rendered. The focused native suites, vet, and JS/WASM cold-mount and image-recovery regressions passed. In the clean local browser build, Anika, Zuri, Samuel, and Rafael message photos matched the People directory; Evelyn's DM, four member photos, and five thread photos loaded, while Ethan retained initials. Opening the emoji picker beside a loaded Samuel photo left its row height and top unchanged. Dark-mode views at 390×844 and 320×740 had matching inner and scroll widths, and photos kept a 34.67 px slot. After runtime promotion, the live chat restored with Mina and Camila photos matching People, an MR initials fallback, and no console warnings or errors. Light mode was not inspected.

## 2026-09-22 (Grouped backlog gate repairs)

- Corrected terminal promotion history: a run that reached the blocked END is closed, while a blocked draft remains correctable. Added terminal/skipped/running-state regression coverage and kept ordinary manager approval fixtures separate from operator diagnostics authority.
- Added application-boundary tests for chat recipient state, current reference discovery, governance records, missing dependencies, and tenant/conversation-bound agent and event access.

- Preserved configured empty page policy, declared chat-media configuration inputs, and moved migration-plan validation behind the application boundary.
- Added correlated observation boundaries for requester notifications and downstream delivery, retaining late-attribute redaction.
- Repaired signing-key, simulation-overlay, durable work-role, immutable-proposal, release and instance-lease regression fixtures without relaxing production guards.
- Added chat action-dispatch, reaction identity, draft-isolation and directional keyboard-resizing regressions; local chat UI coverage is above the unchanged 70% floor.
- Reconciled new backlog test contracts and dependency phases; unfinished notification, agent, document and project features remain open.

## 2026-09-22 (Chat timeline stability)

- Keyed dynamic message, divider, and sidebar siblings so inserting older posts or changing groups preserves existing DOM rows. The timeline MutationObserver now reads scroll geometry once per affected list per mutation callback instead of once per changed row. A focused JS/WASM reconciliation test preserves an existing article after an older post is inserted; a 500-record operation-count test confirms one scroll-height read. These counts do not measure frame rate.
- Fixed click flicker at the route boundary: a chat revalidation rendered the workspace directly while loading but wrapped it in the route component when resolved, so the reconciler remounted the chat DOM. The loading and resolved paths now keep the same component shape. The viewer's emoji reaction is counted once when its RPC completion and stream event arrive in either order; other viewers' counts still update. The JS/WASM route identity regression passed, including its old-behavior negative control, as did the direct local rendering lifecycle cleanup regression; reaction order/replay regressions passed in the targeted native suite.
- Opening the reaction picker on a message without chips now anchors its popup outside message layout and omits a redundant add chip, so opening and closing it does not change timeline height. Added a render regression; the chat UI package passes its full native suite and vet, with 77.5% statement coverage.
- The full `go test -count=1 ./tools/uxqual/cmd/journeywasm/` suite and `go vet ./tools/uxqual/cmd/journeywasm/` passed after these changes. Package-only coverage reported 83.9% of statements; that run printed `ok` before Go hit the documented Windows test-executable cleanup error.
- The combined build was promoted to the live runtime at port 8868; its served WASM hash exactly matched the verified candidate. The original browser reloaded and restored people-ops. On the live runtime, the picker trigger retained focus, the draft stayed empty, and opening and closing an empty picker left timeline height at 6696 and `scrollTop` at 6026.66650390625, with no browser warnings or errors. On the candidate, adding 🔥 counted one and removal succeeded; details open/close and Home → Chat → More actions also passed. Narrow viewport verification for this build remains pending.
- Kept chat revalidations visually quiet; reserved the same bounded image frame before and after media loads, including attachments without dimensions; and carried the reader's scroll position across same-room timeline replacement. A new room still opens at its newest message.
- Verified `go test -count=1 ./internal/humanwork/chatui/ ./tools/uxqual/cmd/journeywasm/`, `go vet ./internal/humanwork/chatui/ ./tools/uxqual/cmd/journeywasm/`, and the targeted JS/WASM scroll-continuity and quiet-refresh regressions under Node. In the live browser, loading two more images left timeline height unchanged and held `scrollTop` at 4518 through draft edits and later reads; Jump to newest reached the bottom. The 390px and 320px views had no horizontal overflow or console errors.

## 2026-09-21 (Capability execution telemetry)

- Added the shared workflow observation boundary to capability execution, with node/instance correlation and terminal outcome/error reporting. Added a regression for failures before the capability gateway is available; capability-runner and workflow-observer package tests pass.

## 2026-09-21 (Chat recipient state, reactions and core audit integrity)

- Enforced recipient-owned read and preference state with current membership and tenant/home checks, monotonic cursors and bounded unread counts. Added authorized, paginated reaction listing and current-membership pin visibility.
- Moved core conversation, membership, post, reaction and pin audit metadata into the canonical chat mutation transaction. A failed audit write rolls back the mutation; an idempotent post replay does not create another audit event. Audit metadata and its digest omit message bodies. Reconciliation now reports a missing core audit projection, and audit rows are immutable.
- App installation, upgrade and status writes now commit their revision CAS and body-free outbox/audit/inventory projection in one tenant transaction. PostgreSQL audit faults roll back both installation and status changes; retries commit once, and a stale status write creates no extra audit. The full chatstore and chatrecordstore PostgreSQL suites previously passed at 74.2% and 82.3% coverage; classification, moderation, race/CI and full pilot gates remain open. Share links are locators and do not persist a share mutation.

## 2026-09-21 (Workflow notifications and approval tracking)

- Fixed accessible text disappearing from floating notification/review disclosures, and corrected narrow-screen Cancel-button sizing. Verified the shared disclosure fix and review Tab/Escape behavior in the Codex browser; added regressions and requester-status storage tests. Updated the bootstrap harness to generated clients after the legacy edge clients were removed; NaaS composed integration/security and authority-refusal checks now pass. The standalone public inbox API and million-row capacity qualification remain incomplete.
- Added a typed, bounded PostgreSQL notification-feed API with deterministic keyset pagination and read/archive/pin/date/workflow filters. New feed indexes reduced the isolated 100,001-row first-page storage p95 from 82.2ms to 0.76ms; this is not end-to-end API latency. Current notification callers reuse the indexed query. Public paginated transport and sustained multi-tenant capacity qualification remain tracked separately.
- Approval pauses now block only their own successors: drain independent READY work before parking, share dispatch across approval/task/timer/signal resumes, and recover committed successors without recording another vote. Added PostgreSQL regressions for rejection, replay, revoked authority and tenant/version fences.
- Fixed the live manager approval handoff when the requester's organization scope differs from the assigned manager's. The exception requires a directly routed, freshly verified current-manager relationship for this case; it grants no wider visibility. Correlated decision logs now include safe phase and reason codes, without proposal or decision text.
- Added the initial NaaS in-app slice: routed approval/task work commits a recipient notification with messaging metadata in the same transaction. Retries deduplicate without resetting read state; rollback removes the notice and metadata.
- Resolve the assigned audience, recheck recipient and current workflow visibility on reads, and expose bounded notifications through the existing journey gRPC service. Notification-read failure does not block the approval list. Publication and reads use correlated workflow telemetry.
- The shared bell now links directly to the employee's workflow and displays the current work-item status in English, German and Arabic. A recorded decision is distinct from overall workflow completion and from external delivery. Email/SMS and inbox-management controls are not part of this slice.
- Integrated People column configuration into Find an employee; fixed narrow-screen sizing and theme/focus-token regressions. Verified desktop and mobile dark layouts and the light People preview. Broader runtime test closure remains blocked by concurrent bootstrap-client changes.

## 2026-09-21 (New employee hire runs through the workflow engine)

- Proved the EXECUTE driver and runtime are workflow-agnostic by running a second workflow through them. `internal/workflow/hireexec` declares New employee hire: offer approval, a background-check callback, an HR review when the check is adverse, candidate forms, three provisioning tasks, a wait until the start date, one authoritative commit and seven exits. `test/workflow` drives it through `execute.Driver` on PostgreSQL with its own step runner, work-item routing, timer factory and terminal writer: the happy path ends with exactly one ledger fact, every call uses a fresh driver so nothing lives in memory between steps, and the rejected-offer and withdrawn-after-adverse-check exits write no fact. No file in `internal/workflow/execute` or `internal/workflow/runtime` changed (`WF-HIRE-001`).
- New employee hire appears in the designer. `execution.PublishReferenceVersions` publishes it as a governed version and the local-development bootstrap approves and activates it beside Promotion, so the landing page lists it under Published workflows, opens its path read-only, and lets an author draft a newer version from it; it is also offered as a template. Standard deployments do not publish it, because nothing serves a hire run yet.
- Workflow notifications are no longer Promotion's alone. `execution.WithWorkNotifications` and `execution.WithRequesterStatusTerminal` wrap any workflow's routing and terminal writer; the hire run asserts one approval or task notice per owner and, for the requester, one update per hand-off and one at the end.
- The limits the hire workflow had to design around are recorded against the todos that own them rather than patched in shared code: provisioning runs in sequence because join arrival counters are not stored (`WF-EXT-019`); the start date reaches the timer through the timer port because a wait's wake time is fixed in the definition (`WF-EXT-012`); the callback is matched on the request id because signal keys are start-time facts (`WF-EXT-014`); the check's verdict is read from the request because step outputs are not resolved on the durable path (`WF-EXT-004`). The hire workflow is not served: the production composition, the intent layer and the journey pages are still Promotion-only (`WF-EXT-007`, `WF-EXT-008`, `WF-EXT-018`).
- Started the engine's main bottleneck, the durable data plane (`WF-EXT-004`): one mapping resolver shared by the simulator and the driver; a run's typed input document and each step's typed outputs stored as append-only artifacts (migration `00322`) in the same transactions as the start and the advancement; resolved inputs handed to the step runner. A run that supplies no input document behaves exactly as before, and Promotion and New employee hire still complete end to end. Context-sourced mappings cannot resolve yet, and no production step reads the resolved inputs until `WF-EXT-005` and `WF-EXT-006`.
- Audit of the 117 open workflow todos: none is closable by verification alone. `WF-REV-015` has passing tests but nothing imports its package, so ledger facts do not yet go through the vault it builds.
- Workflow editor, adversarial review rounds 8 and 9 (7.6 then 7.7 overall; 8.0 then 8.1 for what was built): the folded library gained a "Show all 22 steps" button that opens it in a bounded, scrolling box without moving the path, reachable in the header row down to 320px; the header count now equals what the canvas shows (a loose step's badge includes "not connected", and a draft with no End says so where the path stops); the Exits key keeps its marks at full contrast (3.2:1 to 9.5:1 and above); a Decision says what it chooses between and which values it reads, with its note about rules beside the choices; folded exception results carry one "N not set" instead of a flag per row; "Who is told" opens by saying the person comes from the request's routing rule; the editor dims while a save fences it. One reviewer finding was checked and rejected: exit chips are hidden from assistive technology on purpose, because each row already carries a sentence naming every exit.

## 2026-09-21 (Workflow status notifications and the editor's "Who is told")

- The person who starts a workflow request is now told when it moves. Until now only the owner of routed work was notified (`NAAS-001`); the requester heard nothing. A `WORKFLOW_UPDATE` notice is written to the requester's secure inbox in the same transaction that routes an approval or task, and another in the transaction of the run's terminal write, so a rolled-back advancement leaves no notice and a retry adds none. A requester who also owns the routed work is not told twice, and a request with no recorded requester publishes nothing (`WF-NOTIFY-001`).
- One declaration, `internal/workflow/notifyplan`, states which notices each kernel step type produces. The execution composition publishes from it and the workflow editor reads it, so the editor cannot promise a notice a run does not send. The step inspector gained a "Who is told" section for approval, task and end steps, in en-US, de-DE and ar, and says plainly that notices appear under the bell and are not emailed.
- The bell shows status notices beside approval and task notices, newest first: "Sent to a reviewer", then "Approved and finished", "Declined", "Could not be completed" or "Finished". How a request ended is read from the stage the viewer is already authorized to see, never from stored copy. Each request appears once, at its latest update. A failed status read marks notifications unavailable rather than serving a partial list that reads as complete.
- Status notices use their own message template, so the approval feed, its pagination and its indexes (`NAAS-003`) are untouched, and a status notice can never surface there with a missing work item.
- Workflow editor, from adversarial review round 7: a step's connector no longer carries a second, differently counted amber number beside the step's "to finish" badge; it now says in words which results lead nowhere. Arabic route sentences and step names no longer flip direction (four `dir="auto"` attributes on block-level text were resolving to left-to-right from a leading Latin name). The folded library below 1400px shows a permanent scroll track and a lighter fade so the next row is visible; its heading stacks below 560px; its step count no longer drops by one after the first step is added. Selecting an End explains that an End is a chip, not a row. The History and "to finish" popovers now close on Escape, focus leaving and pointer leaving through the product's shared popover policy. An unnamed result reads "Any other result" rather than "No result".
- Verified live on the uxcheck server: a requester's bell showed "Sent to a reviewer" after starting approval, the finance approver's bell showed the approval request, and after a decline the requester's bell showed "Declined" through the real terminal writer.

## 2026-09-21 (Workflow editor review and redesign)

- Replaced the workflow draft surface with a three-pane editor: a step library, the workflow drawn as one ordered path, and an inspector for the selected step. Published workflows on the landing view use the same path, read-only, so a released version and the draft that succeeds it read the same way. The stage-column graph, the duplicate outline beside it and the stacked catalog/draft/palette page are gone.
- Fixed a wrong-step write. Choosing a step updated only the address bar, so the highlight, `aria-current` and the inspector each kept the step they first rendered, and "Save parameters" could land on a step other than the highlighted one. Selection is now editor state, and the inspector is keyed by the step it edits.
- The path ranks steps by longest route from the start, draws routes that skip or loop on a side rail, names each terminal exit on the steps that reach it, and lists steps nothing leads to under "Not connected yet" instead of ranking them beside the start. A draft no longer shows a run state, and a run the viewer may not see no longer shows every step as "not started".
- Added three durable edit commands end to end (`RemoveWorkflowDraftNode`, `ClearWorkflowDraftOutcome`, `RenameWorkflowDraft`), role-gated with the other designer actions. The editor could previously add steps and routes but not remove them, and a new workflow could not be named. Also refused fragment expansions whose remapped step ids collide.
- Every inspector control saves on commit; the "not connected" choice carries a real value (an empty option value submitted its own label as a step id); refusals are reported by cause (changed elsewhere, not allowed, no access, gone, offline) instead of one generic sentence; each edit has a 20-second bound so one hung call cannot hold the queue.
- Authoring flow: a blank draft offers first steps on the canvas; a step added from the library continues from the selected step's first open usual result and never reroutes a working one; otherwise it waits with a "Connect this step" control; one select sends all unset exception results of a step to an exit.
- Copy and localisation: effect, reversal, lock, overlay, source and compiler-outcome vocabularies now have catalog entries in en-US, de-DE and ar (they fell back to title-cased English tokens that passed the missing-key checks); counted copy uses locale plural rules with all six Arabic categories; block names, group headings and exit names are translated; the generated "Add a approval step." descriptions are replaced by reviewed sentences.
- Responsive and accessibility: three panes to 1400px, then the library folds to a scrolling row of add controls above the path; below 820px an in-page link hops between the path and the step settings; focus-visible rings, a selected state that differs from hover in shape, `aria-invalid` and visible "Not set" on unset routes, logical properties and a mirrored rail in RTL.
- Tests: replaced five test files that pinned the removed markup with behavioural tests for the path model, selection, the inspector, the library, the authoring flow, locale completeness and the exact shipped Promotion definition (every step once, all 72 routes accounted for).

## 2026-09-21 (Configurable People directory columns)

- Added a reusable keyboard-accessible column chooser with Apply/reset and per-user server-persisted choices (`UICOLS-001`). Person stays visible; optional worker number, job code, level, company, business unit and cost center can be searched and sorted without adding sensitive payroll/PII fields.
- Preserve filters, page size and column selections during navigation; return sorting to name when hiding the sorted column. Unsaved choices survive live-data refreshes, and the chooser respects dark-mode surfaces and mobile layouts.
- Added projection, sorting, preference, draft-remount, route-focus and i18n regressions. Rebuilt the Go/WASM dev server and manually verified the live gRPC-backed People page.

## 2026-09-21 (Parallel and join building-block hardening)

- Reject duplicate/blank branch evidence before JOIN aggregation, so replayed results cannot manufacture quorum. Required-set joins now check every mandatory member and reduce failure/unknown states independently of input order (`WF-STEP-021`).
- Prevent integer overflow from bypassing PARALLEL cost admission; refuse over-budget work before any branch starts and retain error telemetry (`WF-STEP-022`).
- Added negative, boundary and permutation regressions; existing JOIN golden outputs and bulk-acknowledgement/leave-return conformance remain green.

## 2026-09-21 (Workflow continuation integrity)

- Fixed signal deduplication so refused deliveries cannot reserve an accepted-delivery identity, suppress a valid retry, or turn repeated late/out-of-order refusals into successful duplicates (`WF-STEP-020`). Refusal audit entries remain intact.
- Verify WAIT requirement and prior-resolution content digests before use, rejecting altered fire times, identities, evidence and replay results while preserving exact replay (`WF-STEP-019`).
- Added failing-before/fixed-after regression cases; both step packages, vet, and Promotion completion/restart/duplicate-wakeup regressions pass locally. No designer changes or workflow publication in this pass.

## 2026-09-21 (Workflow engine hardening)

- Bound caller-held, configured and acquired execution fences to the exact workflow instance; foreign-instance, queue and malformed fences are refused before step execution.
- Isolated guarded step writes with a savepoint so a stale-proposal BLOCKED transition cannot commit effects from the refused advancement.
- Preserved canonical workflow identifiers and late attributes in policy-filtered OTel spans, corrected resumed node trace IDs, and kept nested storage/cancellation failures and cyclic-resume lookup failures visible as failures.
- Added positive, negative and fault regressions, including PostgreSQL rollback proof; runtime/execution and Promotion end-to-end checks passed locally. The full repository commit gate remains blocked by an unrelated concurrent workflow-editor test compilation error.
...
- Completed `WF-EXT-001`: the executable plan's budget-hold compensation now runs inside the advance transaction, so a failed downstream observation releases the hold exactly once. Completed `WF-REV-001`: a cancellation driver discharges recorded compensation obligations in reverse commit order, moving the run to `CANCELLED` or `REPAIR_REQUIRED`. Both packages clear the 70% coverage floor (execution 80.2%, cancellation 78.0%).
- Completed `PROMO-EXEC-002` (tunnel propose/execute/decide/inspect/terminal chain), `PROMO-EXEC-004` (live work-queue claim/decide with separation-of-duties plus pause/resume/cancel/retry branches) and `PROMO-EXEC-005` (sweep, START retry, scheduler loop with recovery), all green on served cells. Completed `REV-003-01` (hand-written edge client deleted; generated capability clients canonical, drift-checked) and `REV-067-01` (page-revision/rollout ledger wired into live page serving). Code-complete but unticked: `WF-EXT-005` (capability runner, 91.6%) waits on `WF-EXT-004`, `WF-REV-015` (per-subject payload vault, 73.6%) waits on `FIELD_LEVEL` disposition rows, and `RBAC-RT-006` server gates pass while the UI fallback removal stays blocked on another session's dirty `productui/model.go`.
- Completed `WF-EXT-002` (workflow registrations with capability-keyed dispatch; prototype/execute digests byte-identical), `WF-REV-002` (one compensate executor for COMPENSATE nodes and governed cancel compensation), `WF-REV-003` (single reversibility declaration sourcing proposals, plans and cancellation) and `WF-REV-004` (shared admission release frees the budget hold on every cancel path; journey-only best-effort release deleted), each with its full matrix green. Ticking `WF-EXT-002` unblocks `WF-EXT-003`/`004`. Not ticked: `REV-003-02` (claimed coordinator files absent from disk at verify time — no evidence to tick), `REV-009-01` (files present, evidence not yet independently re-run).
- Completed `RBAC-RT-004` (Work/Workflow services authorize through the policy decision point; unwired supervision fails closed), `REV-009-02` (all six remaining intervention kinds operator-reachable end to end) and `INTAPI-009` (integration design contract checked against backlog and generated RPCs; 4 live violations caught and fixed), each matrix green on re-runs.
...
- The exhaustive coverage and race lanes are deterministically partitioned across four isolated runners apiece; every shard remains mandatory through the same fail-closed fan-in, cutting validation latency without reducing the package set.
- CICD-001 now verifies the architecture manifest's seven real command roots, replacing the stale forbidden `cmd/admin` expectation with the shipped `cmd/hcmctl` and `cmd/frontenddev` roots.
- The root coverage gate now honors its documented generated-code exclusion for nested `generated` directories and parses Go 1.26's bare coverage summaries for no-test executables. Three thin, library-backed command wrappers carry explicit expiring coverage exceptions instead of being misreported as missing package results.
...
- Domain (`PROMO-EXEC-001`, `PROMO-EXEC-006`): the standard serve profile composes the executing cell by default (engine, authority and scheduler on; digest still required) with the refusing cell kept behind `-execution-authority=false`, and every propose path auto-selects the OPEN catalog vacancy for position-less proposals. Suites green in-tree.
- Data: the tenant-scoped position directory with per-position occupancy (`internal/data/positionfacts`), execution-evidence transaction recording (`evidencestore`), and the new `budgetfacts`/`compfacts` fact packages land with their stores green.
- Rewards (`HIPERF-001`): the governed `hcmnext.rewards.market_rate` port with a static stub table and an off-by-default egress-HTTP skeleton; the execute graph is untouched.
- Product UI (`UXLIVE-001`..`UXLIVE-026`): recorded-outcome wording, history-derived stage tracker, position display, prefilled edit form, failure retry, governed-repair projection, scope counts, brand copy, identifier treatment, request references, vacancy picker propose form, pay-guidance copy, immutable asset caching with Cache Storage fallback, presence rules, one vocabulary for dates, shared action reasons, counted work filters, visible stage states, focus return, grouped row menus, cold-load journey links, person-page states, organization width, shell scroll lock and started-fact intervention availability. The workspace WASM bundle and manifest rebuild with the tree.
- Docs: `planning/todos.md` gains the 38 `PROMO-EXEC`/`UXLIVE`/`HIPERF` entries (29 ticked with evidence, 9 left open), the registry regenerates from markdown, and the archdoc inventory regenerates for the new packages.

Left open with no named tests in the tree: `PROMO-EXEC-002/003/004/005/007` and `HIPERF-002/003/004/005`; their groundwork above is committed without ticks.
...
- Domain engines (`CYCLE-007/008/010`, `ELIG-006`, `SCHED-004`, `ALIGN-030`): future cycle configuration publishes safely, prior cycles correct append-only, multidimensional completion reconciles, eligibility reevaluates on governed-input change, failed trigger firings quarantine and redrive, and the intent app exposes the aligned action plan. All suites green in-tree.
- Domains (`APPT-002`, `ATTEND-003`, `BAL-006/007/008`, `CASE-002`, `CBA-003`, `CRM-003/005`, `CUSTOM-007`, `ER-002`, `MATCH-005`, `APP-006`, `PAYMETHOD-003`, `PAYRUN-003/007`, `PRIV-006`, `QUAL-003/005`, `RECRUIT-001`, `SCENARIO-004`, `SCHED-OPT-003`, `SUB-005/006`, `TENANT-005`, `PAYINPUT-002`): participant/availability resolution, meal-break and overtime exceptions, retroactive balance correction with dependent recalculation and derivation explanation, compartmented case notes, CBA impact with representation work, campaign/audience and prospect conversion, custom-object versioning, appeal-safe employee relations, fairness-constrained matching, marketplace certification, pay-method verification/acceptance/settlement distinction, payroll input lock and effect release, subject-request resolution, qualification validity and reevaluate-on-expiry, recruiting lifecycles, scenario comparison, schedule hard constraints, subscription retry/replay, and epoch-fenced tenant relocation. Each package passes its matrix in-tree.
- Trust (`PRIV-008/009`, `PROCESS-001`): the federal-tax-information boundary, the financial-breach notification matrix and the privacy-scoped process-mining log land with golden/security/integration/mutation proofs.
- Operations and messaging (`FORENSIC-001`, `MAIL-001`, `ALIGN-056/060`, `FULFILL-001`, `ALIGN-018`, `NEXT-007`): forensic acquisition with chain of custody, authenticated email domains with bounce/complaint/suppression lifecycle, release-evidence chronology and rollback, physical-notice fulfillment, the product-query compile path and the Medical Leave conformance firewall. All suites green in-tree.
- Policy and planning tools (`ALIGN-004`, `SUPPLY-004`): the temporal-vocabulary alignment gate and the vulnerability-severity patch-SLA clock land with golden/security/integration proofs; the coverage-gate exceptions record improved statement coverage (appointment 61.6 to 64.2, cba 42.6 to 57.3, employeerelations 52.3 to 68.6, hrcase 61.4 to 67.6) and the archdoc inventory regenerates for the new packages.
- Product UI (`UIPOLISH-009/010`, `UXAUDIT-002/009/010/013`): stable loading/empty/error/success states, the icon-bypass guard across render callsites, the discoverable promotion path with workflow/workspace/productui matrix green, and blind-audit remediation for journeys 009/010/013. The workspace WASM bundle and manifest rebuild with the tree.

Deferred to a later round (ticked where contract-green, code still in the working tree): the 26 reviewed state packs plus `LEGAL-005/007/014` (their primaries pass; the `internal/governance/legal` package stays red on `LEGAL-016` goldens/conformance and on new tests for not-yet-migrated states, so no legal Go files or packs are committed here); `LEGAL-016` was unticked after its golden/conformance tests failed against the updated packs. In-flight work for open todos stays uncommitted throughout: `ABUSE-006`, `ANON-006/008`, `SEARCH-002`, `BEN-003`, `CLOCK-003`, `GARN-001`, `METRIC-001`, `PERSIST-JOBARCH-002`, `KNOW-003/004`, `SETTLE-004`, `FEATURE-CONF-001`, `UIPOLISH-002/003/005/006`, `UXSCAN-008`, the LA/MA/MD/ME/MI/MN state packs and the MI-matrix conflict.
...
- Remote table feedback (WEB-246): reusable data tables can retain their last authorized rows and stable geometry while an in-place, localized spinner and status announce sorting, paging, or page-size work. People directory transitions now share the authoritative route loader, including cancellation and preference persistence, rather than resolving sort changes through optimistic-only state.
- Keyboard operability (WEB-245): every registered product page now enters one parsed-DOM keyboard contract in English, German and Arabic. Page utilities use the shared modal focus trap, and the trap excludes hidden, inert and non-rendered responsive controls so Tab and Shift+Tab wrap only across visible targets before Escape restores the exact trigger.
- Scalable page extensions (WEB-242-244): one immutable page-module registry now owns rendering, access audience, stable feature identity and reusable route/data profiles. Server, shell, Go client and WASM navigation consume those profiles, while a registry-wide contributor gate proves authorization, accessibility, theming, responsive behavior and bounded lookup cost for every registered page. The live promotion review trigger now exposes exactly one state label and avoids duplicating an already-localized `Review and submit` prefix.
...
## 2026-09-22 (Team and project channel widgets)

- Added channel-scoped informal team roles and project planning milestones, plus manager-only pin and unpin actions for team/project channel widgets. Focused backend, channel record/recovery, and UI tests passed. On r17 at port 8888, browser QA scored 9.1/10.
- Limits: Rafael's member account could not be used for live pin/unpin verification. Retention purge remains dry-run only. The full suite still has unrelated failures, so this focused evidence is not a full-suite pass.
