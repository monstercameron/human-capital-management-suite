...
func (h *journeyHarness) approverCtx(t *testing.T) context.Context {
	t.Helper()
	return h.ctxAs(t, journeyApprover, journeyOperatorRoles()...)
}
...
func TestJourneyDecideApproveCompletesWithOneGovernedWrite(t *testing.T) {
	...
	// own claim, start and decision name the assigned reviewer who pressed the button.
	decided := false
	for _, tr := range detail.Transitions {
		if strings.HasPrefix(tr.Reason, "journey.") && tr.Actor != journeyApprover {
			t.Errorf("transition %s -> %s (%s) names actor %q, want the assigned reviewer %q",
...
func TestJourneyDecideTwiceIsAStageRefusal(t *testing.T) {
	...
	approverCtx := h.approverCtx(t)
	first, err := h.engine.Decide(approverCtx, proposed.IntentID, workspace.Decision{Approve: true, Reason: "yes"})
	if err != nil {
		t.Fatalf("Decide: %v", err)
	}
	replay, err := h.engine.Decide(approverCtx, proposed.IntentID, workspace.Decision{Approve: true, Reason: "yes"})
	if err != nil {
	...
	if _, err := h.engine.Decide(approverCtx, proposed.IntentID, workspace.Decision{Approve: false, Reason: "changed my mind"}); err == nil {
