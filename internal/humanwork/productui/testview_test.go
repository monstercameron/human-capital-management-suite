...
func testView(page PageID) View {
	...
	// Keep the shared fixture bound to the worker represented by the open
	// journey. My Work is intentionally scoped by the viewer/assignee
	// identity, so an unbound fixture would make every page render empty.
	view.Viewer = ViewerProfile{PersonID: "worker-jordan", Name: "Taylor", Initials: "T"}
	view.Work = []WorkItem{
		{ID: "intent-1", Initials: "JL", Title: "Promotion journey", Person: "Jordan Lee", PersonRef: "worker-jordan", AssigneeRef: "worker-jordan", Summary: "ENG2 G6 → ENG3 G7", Status: "Awaiting approval", Due: "2026-09-15", Tone: "warning", Href: "/workspace/app/journeys?journey=intent-1", EffectiveDate: "2026-09-15",
	...
	view.PersonWorkflows = []PersonWorkflow{
		{ID: "promotion", Name: "Promotion", Category: "Career & compensation", Description: "Request a change to job, grade, position, or compensation.", Href: "/workspace/app/journeys?mode=new&worker=worker-avery", LaunchHref: func(person string) string { return "/workspace/app/journeys?mode=new&worker=" + person }},
	...
	view.LauncherActions = []LauncherActionProjection{{
		ID: SemanticActionPromoteWorker, State: ActionState{Availability: ActionAvailable},
	}}
