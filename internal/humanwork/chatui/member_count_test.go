...
func TestOneMemberDirectMessageUsesSingularCount(t *testing.T) {
	...
	markup := render(t, m)
	if !strings.Contains(markup, "Direct message · 1 member") || strings.Contains(markup, "1 members") {
		t.Fatalf("self DM count was not singular: %s", markup)
	}
	m.Conversations[0].MemberCount = 2
	if markup = render(t, m); !strings.Contains(markup, "Direct message · 2 members") {
