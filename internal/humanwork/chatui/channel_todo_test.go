...
func TestChannelTodoAndPinsRenderIndependentlyOfTimeline(t *testing.T) {
	...
	if markup = render(t, m); !strings.Contains(markup, `class="channel-todo-trigger"`) || strings.Contains(markup, "channel-todo-inline") {
...
func TestChannelTodoCompletionNameAndGuestReference(t *testing.T) {
	...
	if !strings.Contains(render(t, m), "Completed by channel member") {
...
func TestChannelTodoNewTaskPolicyAndSelectedMember(t *testing.T) {
	...
	if !strings.Contains(render(t, m), `<option selected value="EVERYONE"`) {
...
func TestChannelTodoZeroOpenHasReadableCount(t *testing.T) {
	...
	if markup := render(t, m); !strings.Contains(markup, "No open tasks") || strings.Contains(markup, "> open<") {
