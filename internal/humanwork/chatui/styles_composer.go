...
const composerPolishStyles = `.thread-composer{position:relative}` +
	...
	`.mention-option.active{background:var(--accent);color:var(--hcm-color-on-brand)}.mention-option.active .mention-detail{color:inherit;opacity:.85}` +
	...
	`.chat-row.selected,.chat-row.selected:hover{background:var(--accent);color:var(--hcm-color-on-brand)}.chat-row.selected::before{display:none}.chat-row.selected .kind-glyph{opacity:1}.chat-row.selected .chat-badge{background:var(--hcm-color-on-brand);color:var(--accent)}` +
	...
	`.chat-rail-row{position:relative}.chat-rail-row .rail-row-more{position:absolute;inset-inline-end:2px;top:2px;height:28px;width:26px;z-index:1}.chat-rail-row:has(.chat-row.selected) .rail-row-more{color:var(--hcm-color-on-brand)}.chat-rail-row:hover .chat-badge,.chat-rail-row:focus-within .chat-badge{visibility:hidden}` +
	...
	`.chat-row.selected,.chat-row.selected:hover{background:color-mix(in srgb,var(--accent) 28%,var(--canvas));color:var(--ink)}.chat-row.selected .chat-badge{background:var(--accent);color:var(--hcm-color-on-brand)}.chat-rail-row:has(.chat-row.selected) .rail-row-more{color:var(--ink)}` +
	...
	`@media(pointer:coarse){.message-action,.reaction.add,.conversation-header .icon-button,.conversation-header .channel-todo-trigger,.conversation-header .channel-poll-trigger,.thread-more,.tool-button{min-width:44px;min-height:44px}}` +
	...
	`.chat-search-results>h2{position:absolute;clip:rect(0 0 0 0);clip-path:inset(50%);width:1px;height:1px;overflow:hidden;white-space:nowrap}.search-result-meta strong{font-weight:700}` +
	`@container chatmain (max-width:560px){.conversation-header .icon-button,.conversation-header .channel-todo-trigger,.conversation-header .channel-poll-trigger,.conversation-header .rail-pill{border:0;background:transparent;width:36px;min-width:36px;height:36px;padding:0;justify-content:center;border-radius:var(--hcm-radius-control)}.format-tools{display:flex}.format-button[data-extra=code],.format-button[data-extra=bullets],.format-button[data-extra=quote]{display:none}}` +
	...
	// Round-11 critique.
	`.chat-composer .composer-help{position:static;margin-inline-start:auto;margin-top:0;font-size:.75rem}.chat-composer .composer-help+.send-button{margin-inline-start:8px}@container chatmain (max-width:560px){.chat-composer .composer-help{display:none}}` +
	`.chat-workspace[dir="rtl"] .thread-pane .message-body{text-align:match-parent}` +
	`@media (hover:none),(max-width:760px){.thread-root .thread-view-in-channel{inset-inline-end:44px;top:3px}}` +
	...
	`@container chatmain (max-width:560px){.rail-pill>span:not(.rail-pill-dot),.channel-todo-trigger-label,.channel-poll-trigger-label{display:none}.rail-pill{padding:0 9px}.conversation-title{min-width:96px}}`
