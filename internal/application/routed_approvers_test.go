...
func routedApproverContexts(t *testing.T, verifier *trust.HMACVerifier, cfg ServeConfig, at time.Time) (finance, manager context.Context) {
	...
	financeSubject := cfg.ExecutionFinancePartner
	if financeSubject == "" {
		var err error
	...
	managerSubject := cfg.ExecutionManagerApprover
	if managerSubject == "" {
		var err error
	...
	if financeSubject == managerSubject {
		t.Fatal("the two configured approval classes must have distinct principals")
	}
